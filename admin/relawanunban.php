...
$query = "SELECT id_relawan, nama, username, alamat, email, status FROM relawan WHERE status = 'ban'";
...
<script>
    function updateStatus(id_relawan, action) {
        // Lakukan AJAX request untuk mengubah nilai kolom status menjadi "unban"
        ...
        .then(data => {
            if (data.success) {
                alert('Berhasil memUnban pengguna!');
                location.reload();
            } else {
                alert('Gagal memUnban pengguna!');
            }
        })
